...
function defaultGoalDates(period) {
  ...
  return { startDate: toISODate(startOfWeekMonday(now)), endDate: toISODate(endOfWeekSunday(now)) };
}
...
function renderGoalDetail(userName, goal, plan, canEditEarnings) {
  ...
  return `
    <div class="card catalog-toolbar">
      <div class="catalog-toolbar-row">
        <div>
          ...
          <p class="catalog-hint">${periodLabel}: ${escapeHtml(goal.startDate)} — ${escapeHtml(goal.endDate)} · основные смены: ${shifts} · доп: ${extras}</p>
        </div>
    ...
    ${canEditEarnings ? `
    <div class="card" style="padding:14px 16px;margin-bottom:14px">
      <strong>Внести заработок за день</strong>
      <div class="catalog-filters" style="margin-top:10px">
        ...
        <button class="btn btn-primary btn-sm" data-action="save-goal-earn" data-user="${escapeAttr(userName)}">Сохранить день</button>
      </div>
      <p class="field-hint">Если день недобрал план — остаток автоматически переносится на следующие смены.</p>
    </div>` : ''}
        ...
        ${plan.days.map(d => {
          ...
          return `
            <div class="goal-day ${d.status} ${d.isToday ? 'today' : ''}">
              ...
              <div class="goal-day-right">${body}</div>
            </div>`;
...
function showGoalEditor(userName) {
  ...
  const main = new Set((existing?.mainShifts || [1, 2, 3, 4, 5]).map(Number));
