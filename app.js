...
async function toggleCallPlay(id) {
  const card = document.querySelector(`.call-player[data-player-for="${CSS.escape(id)}"]`);
  if (!card) return;
  ...
  if (!audio._bound) {
    ...
    audio.addEventListener('loadedmetadata', () => {
      ...
      // update meta duration if missing
      loadCallsMeta();
      const item = (state.calls || []).find(c => c.id === id);
...
function handleClick(e) {
  ...
  switch (action) {
    ...
    case 'toggle-call-play': toggleCallPlay(el.dataset.id); break;
