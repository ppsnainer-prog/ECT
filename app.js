...
function applyCloudRecord(remote) {
  ...
  if (Array.isArray(ex.cars)) {
    state.cars = ex.cars;
    try { if (typeof persistCarsLocal === 'function') persistCarsLocal(); } catch (_) {}
  }
  if (Array.isArray(ex.calls)) {
    state.calls = ex.calls;
    try { if (typeof persistCallsMeta === 'function') persistCallsMeta(); } catch (_) {}
  }
  if (ex.goalsStore && typeof ex.goalsStore === 'object') {
    state.goalsStore = ex.goalsStore;
    try {
      if (typeof persistGoalsStore === 'function') persistGoalsStore();
      else localStorage.setItem('ect_goals_v1', JSON.stringify(state.goalsStore));
    } catch (_) {}
  }
  if (Array.isArray(ex.refInfo)) {
    state.refInfo = ex.refInfo;
    try { if (typeof persistRefInfo === 'function') persistRefInfo(); } catch (_) {}
  }
  ...
  try {
    if (imgInput?.files?.length) {
      for (const file of imgInput.files) {
        ...
        await idbPutAudio(mid, blob);
        item.images.push({ id: mid, name: file.name || 'image', type: blob.type || 'image/png', size: blob.size || 0 });
      }
    }
    if (audInput?.files?.length) {
      for (const file of audInput.files) {
        const mid = REF_MEDIA_PREFIX + uid();
        const blob = file instanceof Blob ? file : new Blob([file], { type: file.type || 'audio/mpeg' });
        await idbPutAudio(mid, blob);
        item.audio.push({ id: mid, name: file.name || 'audio', type: blob.type || 'audio/mpeg', size: blob.size || 0 });
      }
    }
  } catch (e) {
    console.warn(e);
        ...
        imageHtml.push(
          `<figure class="ref-media">
             <a href="${url}" target="_blank" rel="noopener">
               <img src="${url}" alt="${escapeAttr(img.name || '')}" class="ref-img" loading="lazy">
             </a>
  ...
  for (const a of (item.audio || [])) {
    try {
      ...
      if (blob) {
        ...
        </div><audio controls preload="metadata" src="${url}" style="width:100%"></audio></div>`);
      } else {
  ...
  const cards = list.map(m => {
    ...
    if (kind === 'image' && m.url) {
      preview = `<a href="${escapeAttr(m.webViewLink || m.url)}" target="_blank" rel="noopener">
        <img class="script-media-thumb" src="${escapeAttr(m.url)}" alt="${escapeAttr(m.name || '')}" loading="lazy">
      </a>`;
    } else if (kind === 'audio' && m.url) {
      preview = `<audio class="script-media-audio" controls preload="none" src="${escapeAttr(m.url)}"></audio>`;
    } else if (kind === 'video' && m.url) {
      preview = `<video class="script-media-video" controls preload="metadata" src="${escapeAttr(m.url)}"></video>`;
    } else {
