...
async function showRefView(id) {
      ...
      if (blob) {
        ...
        imageHtml.push(
          `<figure class="ref-media">
             ...
             <figcaption>${escapeHtml(img.name || 'Изображение')}</figcaption>
           </figure>`
        );
      } else {
  ...
  for (const a of (item.audio || [])) {
    try {
      ...
      if (blob) {
        const url = URL.createObjectURL(blob);
        audioHtml.push(`<div class="ref-audio"><div class="field-hint">${escapeHtml(a.name || 'Аудио')}</div><audio controls preload="metadata" src="${url}" style="width:100%"></audio></div>`);
      } else {
