...
function renderHome() {
  ...
  if (q) {
    ...
    scripts = [...scripts].sort((a, b) => (b.opens || 0) - (a.opens || 0) || b.updatedAt - a.updatedAt).slice(0, 8);
    otabotki = otabotki.slice(0, 8);
  }
  ...
  otabotki = otabotki.slice(0, q ? 20 : 8);
    ...
    <div class="home-quick">
      <div class="home-search-wrap card">
        ...
        <input type="search" class="home-search-input" id="homeSearch"
          placeholder="Скрипт или отработка: название, категория, текст…"
          value="${escapeAttr(state.homeQuery || '')}" autocomplete="off" />
        <div class="home-search-meta">
          <span>${total} скриптов · ${otCount} отработок</span>
          ${q ? `<span>Найдено: <b>${scripts.length}</b> скриптов, <b>${otabotki.length}</b> отработок</span>` : '<span>Начните вводить — без перехода по вкладкам</span>'}
        </div>
        <div class="home-quick-actions">
          <button class="btn btn-sm btn-outline" data-action="nav" data-page="scripts">Все скрипты</button>
          <button class="btn btn-sm btn-outline" data-action="nav" data-page="otabotki">Все отработки</button>
      ...
      <div class="home-results">
        <section class="card home-col">
          <div class="section-title">
            <span>📜 Скрипты ${q ? '' : '(частые / недавние)'}</span>
            ${!q ? '<button class="btn btn-sm btn-outline" data-action="nav" data-page="scripts">Все →</button>' : ''}
          </div>
          ${scripts.length === 0
            ? `<div class="empty-state" style="padding:20px"><p>${q ? 'Скриптов не найдено' : 'Пока нет скриптов'}</p></div>`
            : `<div class="item-list">${scripts.map(s => `
                <div class="item-card card-interactive" data-action="open-script" data-id="${s.id}">
                  <h4>${escapeHtml(s.title)}</h4>
                  <p style="font-size:0.82rem;margin:0">
                    ${s.category ? `<span class="badge badge-primary">${escapeHtml(s.category)}</span> ` : ''}
                    ${(s.otabotkiIds || []).length ? `🔄 ${(s.otabotkiIds || []).length} · ` : ''}
                    ${(s.opens || 0) ? `👁 ${s.opens}` : formatDate(s.updatedAt)}
                  </p>
                </div>
              `).join('')}</div>`
          }
        ...
        <section class="card home-col">
          <div class="section-title">
            <span>🔄 Отработки</span>
            ${!q ? '<button class="btn btn-sm btn-outline" data-action="nav" data-page="otabotki">Все →</button>' : ''}
          </div>
          ${otabotki.length === 0
            ? `<div class="empty-state" style="padding:20px"><p>${q ? 'Отработок не найдено' : 'Пока нет отработок'}</p></div>`
            : `<div class="item-list">${otabotki.map(o => `
                <div class="item-card ${o.scripts[0] ? 'card-interactive' : ''}" ${o.scripts[0] ? `data-action="open-script" data-id="${o.scripts[0].id}"` : ''}>
                  <h4>${escapeHtml(o.title)}</h4>
                  <p style="font-size:0.82rem;margin:0;line-height:1.45">
                    ${o.categories.slice(0, 3).map(c => `<span class="badge badge-primary">${escapeHtml(c)}</span>`).join(' ') || '<span class="badge">без категории</span>'}
                    ${o.scripts.length ? ` · ${o.scripts.slice(0, 2).map(s => escapeHtml(s.title)).join(', ')}${o.scripts.length > 2 ? '…' : ''}` : ''}
                  </p>
                  ${o.text ? `<p style="font-size:0.8rem;color:var(--text-muted);margin:6px 0 0;white-space:pre-wrap">${escapeHtml(o.text.slice(0, 100))}${o.text.length > 100 ? '…' : ''}</p>` : ''}
                </div>
              `).join('')}</div>`
          }
        </section>
    ...
    ${list.length === 0
      ...
      : `<div class="card-grid">${list.map(item => `
          <div class="card">
            ...
            <div class="script-meta" style="margin-top:8px;flex-wrap:wrap;gap:6px">
              ...
              ${item.childrenCount ? `<span class="badge">↳ ${item.childrenCount}</span>` : ''}
            </div>
            ${item.scripts.length ? `
              <p style="font-size:0.8rem;color:var(--text-muted);margin-top:8px;line-height:1.45">
                ${item.scripts.slice(0, 5).map(s =>
                  `<a href="#" data-action="open-script" data-id="${s.id}" style="color:var(--primary)">${escapeHtml(s.title)}</a>${s.category ? ' <span style="opacity:.7">[' + escapeHtml(s.category) + ']</span>' : ''}`
                ).join(' · ')}
                ${item.scripts.length > 5 ? ' …' : ''}
              </p>` : '<p style="font-size:0.8rem;color:var(--text-muted);margin-top:8px">Пока не привязана к скриптам</p>'}
            ${item.text ? `<p style="font-size:0.88rem;color:var(--text-muted);margin-top:10px;white-space:pre-wrap;word-break:break-word">${escapeHtml(item.text.slice(0, 200))}${item.text.length > 200 ? '…' : ''}</p>` : ''}
            ${canChange ? `<div class="actions-row" style="margin-top:12px">
              ...
              <button class="btn btn-outline btn-sm" data-action="attach-otabotka-prompt" data-id="${item.id}" title="Добавить в скрипт">➕ в скрипт</button>
