    ...
    <div class="home-quick">
      <div class="home-search-wrap card">
        ...
        <div class="home-search-meta">
          <span>${total} скриптов · ${otCount} отработок · ${CAR_BRANDS.length} марок</span>
          ${q
      ...
      <div class="home-results home-results-3">
        ...
        <section class="card home-col">
          ...
          ${cars.length === 0
            ? `<div class="home-empty">${q ? 'Марка не найдена — попробуйте тег (тайота, бмв)' : 'Введите марку'}</div>`
            : `<div class="home-list home-cars">${cars.map(c => `
                <button type="button" class="home-row home-car-row" data-action="nav" data-page="catalog" title="Теги: ${(c.tags || []).slice(0, 4).join(', ')}">
                  <span class="home-row-title">${escapeHtml(c.name)}</span>
                  <span class="home-row-meta home-car-tags">${(c.tags || []).slice(0, 3).map(t => `<span class="tag-chip">${escapeHtml(t)}</span>`).join('')}</span>
                </button>
              `).join('')}</div>`
          }
        </section>
