/**
 * ЕЦТ Скрипты v2.5 — общие отработки по категориям + локальный автокаталог
 */
...
let state = {
  scripts: [],
  // Общая библиотека отработок: одна запись → много скриптов одной/нескольких категорий
  ...
  otabotkiTab: 'all', // all | shared | private
  expandedNodes: {},
  collapsedBlocks: {}
};
...
function renderScriptDetail() {
  ...
  const sharedForScript = getSharedForScript(script);
  ...
  const sharedCollapsed = state.collapsedBlocks['shared_ot_' + script.id] === true;
        ...
        <aside class="crm-side">
          ...
          <section class="crm-block">
            <div class="crm-block-head crm-block-head-teal" data-action="toggle-block" data-key="otabotki_${script.id}">
              <span>🔄 Свои отработки <span class="badge badge-teal">${countTree(otabotki)}</span></span>
              <span>
          ...
          <section class="crm-block">
            <div class="crm-block-head crm-block-head-teal" data-action="toggle-block" data-key="shared_ot_${script.id}" style="background:linear-gradient(135deg,#0f766e,#115e59)">
              <span>📚 Общие (${escapeHtml(script.category || '—')}) <span class="badge badge-teal">${sharedForScript.length}</span></span>
              <span>
                <button class="btn btn-sm btn-ghost" data-action="nav" data-page="otabotki" title="Библиотека">↗</button>
                <span class="expand-icon ${sharedCollapsed ? '' : 'open'}">${sharedCollapsed ? '▼' : '▲'}</span>
              </span>
            </div>
            <div class="crm-block-body ${sharedCollapsed ? 'collapsed' : ''}">
              ${sharedForScript.length === 0
                ? `<div class="crm-empty">Нет общих отработок для категории «${escapeHtml(script.category || '—')}». Добавьте в разделе Отработки.</div>`
                : sharedForScript.map(o => `
                    <div class="crm-item" style="border-left:3px solid #14b8a6">
                      <div class="crm-item-head" data-action="toggle-item-text" data-id="shared_${o.id}">
                        <span class="crm-item-title">📚 ${escapeHtml(o.title)}</span>
                        <span class="expand-icon">${state.expandedNodes['shared_' + o.id] === false ? '▼' : '▲'}</span>
                      </div>
                      <div class="crm-item-body ${state.expandedNodes['shared_' + o.id] === false ? 'collapsed' : ''}" style="white-space:pre-wrap;font-size:0.88rem">${escapeHtml(o.text || '')}</div>
                    </div>
                  `).join('')}
            </div>
          </section>
...
/** Общие отработки, подходящие скрипту по категории */
function getSharedForScript(script) {
  const cat = (script && script.category || '').trim();
  if (!cat) return [];
  return (state.sharedOtabotki || []).filter(o =>
    (o.categories || []).some(c => String(c).trim().toLowerCase() === cat.toLowerCase())
  );
}
...
function renderOtabotkiCatalog() {
  ...
  const tab = state.otabotkiTab || 'all';
  const canChange = canEdit();

  let privateList = collectAllOtabotki();
  const sharedList = (state.sharedOtabotki || []).map(o => ({
    id: o.id,
    title: o.title,
    text: o.text || '',
    categories: o.categories || [],
    category: (o.categories || []).join(', ') || 'Без категории',
    source: 'shared',
    children: o.children || []
  }));

  const categories = [...new Set([
    ...privateList.map(x => x.category),
    ...sharedList.flatMap(x => x.categories.length ? x.categories : ['Без категории']),
  ...
  if (catFilter) {
    privateList = privateList.filter(x => x.category === catFilter);
  }
  if (scriptFilter) privateList = privateList.filter(x => x.scriptId === scriptFilter);
  if (q) {
    privateList = privateList.filter(x =>
  ...
  let filteredShared = sharedList;
  if (catFilter) {
    filteredShared = filteredShared.filter(x =>
      (x.categories || []).includes(catFilter) || (!x.categories.length && catFilter === 'Без категории')
    );
  }
  if (q) {
    filteredShared = filteredShared.filter(x =>
  ...
  return `
    <div class="card" style="margin-bottom:16px;padding:14px 16px">
      <div style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;justify-content:space-between">
        <div>
          <strong>Общая библиотека отработок</strong>
          <p style="margin:4px 0 0;font-size:0.85rem;color:var(--text-muted)">
            Одна отработка → все скрипты выбранных категорий. Правка здесь обновляет везде.
          </p>
        </div>
        ${canChange ? `<button class="btn btn-primary btn-sm" data-action="add-shared-otabotka">+ Общая отработка</button>` : ''}
      </div>
    </div>
    ...
    <div class="search-bar" style="flex-wrap:wrap">
      ...
      <select class="search-input" id="otabotkiScriptFilter" style="flex:0 0 180px;cursor:pointer">
      ...
      <div style="display:flex;gap:6px">
        <button class="btn btn-sm ${tab === 'all' ? 'btn-primary' : 'btn-outline'}" data-action="otabotki-tab" data-tab="all">Все</button>
        <button class="btn btn-sm ${tab === 'shared' ? 'btn-primary' : 'btn-outline'}" data-action="otabotki-tab" data-tab="shared">Общие (${sharedList.length})</button>
        <button class="btn btn-sm ${tab === 'private' ? 'btn-primary' : 'btn-outline'}" data-action="otabotki-tab" data-tab="private">В скриптах (${privateList.length})</button>
      </div>
    </div>

    ${showShared ? `
      <div class="section-title" style="margin:18px 0 10px"><span>📚 Общие отработки</span>
        <span style="font-size:0.85rem;color:var(--text-muted);font-weight:400">${filteredShared.length}</span>
      </div>
      ${filteredShared.length === 0
        ? '<div class="empty-state" style="padding:24px"><p>Нет общих отработок. Создайте одну и укажите категории.</p></div>'
        : `<div class="card-grid">${filteredShared.map(item => `
            <div class="card">
              <h3 style="font-size:1rem">${escapeHtml(item.title)}</h3>
              <div class="script-meta" style="margin-top:8px;flex-wrap:wrap">
                <span class="badge badge-teal">📚 общая</span>
                ${(item.categories || []).map(c => `<span class="badge badge-primary">${escapeHtml(c)}</span>`).join('') || '<span class="badge">Без категории</span>'}
              </div>
              ${item.text ? `<p style="font-size:0.88rem;color:var(--text-muted);margin-top:10px;white-space:pre-wrap;word-break:break-word">${escapeHtml(item.text.slice(0, 220))}${item.text.length > 220 ? '…' : ''}</p>` : ''}
              ${canChange ? `<div class="actions-row" style="margin-top:12px">
                <button class="btn btn-outline btn-sm" data-action="edit-shared-otabotka" data-id="${item.id}">✏️</button>
                <button class="btn btn-outline btn-sm" data-action="sync-shared-to-category" data-id="${item.id}" title="Скопировать в скрипты категорий">↪ в скрипты</button>
    ...
    ${showPrivate ? `
      <div class="section-title" style="margin:22px 0 10px"><span>📜 Отработки внутри скриптов</span>
        <span style="font-size:0.85rem;color:var(--text-muted);font-weight:400">${privateList.length}</span>
      </div>
      ${privateList.length === 0
        ? '<div class="empty-state" style="padding:24px"><p>Приватных отработок не найдено</p></div>'
        : `<div class="card-grid">${privateList.map(item => `
...
function renderCatalog() {
  // Локальный catalog.html рядом с сайтом — без долгой загрузки чужого origin
  const localSrc = 'catalog.html';
  return `
    <div style="margin-bottom:14px;display:flex;gap:10px;flex-wrap:wrap;align-items:center">
      <a class="btn btn-outline btn-sm" href="${localSrc}" target="_blank" rel="noopener">Открыть в новой вкладке ↗</a>
      <a class="btn btn-outline btn-sm" href="https://kowmarikk0-max.github.io/car/" target="_blank" rel="noopener">Оригинал (GitHub) ↗</a>
      <span style="font-size:0.85rem;color:var(--text-muted)">Файлы каталога на сайте — быстрее и можно править</span>
    </div>
    <iframe class="catalog-frame" src="${localSrc}" title="Автокаталог"></iframe>
  `;
}
...
function importData(file) {
  ...
  reader.onload = async (e) => {
    try {
      ...
      if (Array.isArray(data.sharedOtabotki)) state.sharedOtabotki = data.sharedOtabotki;
      await saveData();
      toast(`Импортировано: ${state.scripts.length} скриптов, ${(state.sharedOtabotki || []).length} общих отработок`);
      navigate('scripts');
    } catch (err) {
...
function handleClick(e) {
  ...
  const editActions = new Set([
    ...
    'delete-shared-otabotka', 'sync-shared-to-category'
  ]);
  ...
  switch (action) {
    ...
    case 'otabotki-tab':
      state.otabotkiTab = el.dataset.tab || 'all';
    ...
    case 'sync-shared-to-category': syncSharedToScripts(el.dataset.id); break;
  }
}

/* ========== Shared otabotki (library) ========== */
function showSharedOtabotkaModal(id) {
  ...
  const catsHtml = cats.length
    ...
    : '<p style="color:var(--text-muted);font-size:0.9rem">Сначала задайте категории скриптам — они появятся здесь.</p>';
  openModal(
    item ? 'Редактировать общую отработку' : 'Новая общая отработка',
    `<div class="form-group"><label>Заголовок</label><input type="text" id="fSharedTitle" value="${escapeAttr(item ? item.title : '')}" placeholder="Например: Нет в наличии"></div>
     ...
     <div class="form-group"><label>Категории (к каким скриптам относится)</label>
       <div style="max-height:160px;overflow:auto;border:1px solid var(--border);border-radius:8px;padding:8px 12px">${catsHtml}</div>
       <p style="font-size:0.8rem;color:var(--text-muted);margin-top:6px">Можно ввести новую категорию вручную:</p>
       <input type="text" id="fSharedCatsExtra" placeholder="Медицина, Авто..." value="">
     </div>`,
    `<button class="btn btn-outline" data-action="close-modal">Отмена</button>
...
async function saveSharedOtabotka(id) {
  ...
  const saved = await saveData();
  closeModal();
  toast(saved ? 'Общая отработка сохранена' : 'Сохранено локально', saved ? undefined : 'error');
  render();
}

async function deleteSharedOtabotka(id) {
  if (isCommonAccount()) return;
  if (!confirm('Удалить общую отработку из библиотеки?')) return;
  state.sharedOtabotki = (state.sharedOtabotki || []).filter(x => x.id !== id);
  const saved = await saveData();
  toast(saved ? 'Удалено' : 'Удалено локально');
  render();
}

/** Скопировать общую отработку в каждый скрипт указанных категорий (одноразово) */
async function syncSharedToScripts(id) {
  if (isCommonAccount()) return;
  const item = (state.sharedOtabotki || []).find(x => x.id === id);
  if (!item) return;
  const cats = (item.categories || []).map(c => c.toLowerCase());
  if (!cats.length) { toast('У отработки нет категорий', 'error'); return; }
  let added = 0;
  for (const s of state.scripts) {
    const sc = (s.category || '').trim().toLowerCase();
    if (!cats.includes(sc)) continue;
    if (!Array.isArray(s.otabotki)) s.otabotki = [];
    const already = s.otabotki.some(o =>
      o.sharedId === item.id || (o.title === item.title && (o.text || '') === (item.text || ''))
    );
    if (already) continue;
    s.otabotki.push({
      id: uid(),
  ...
  if (!added) { toast('Уже есть во всех скриптах категорий или нет скриптов'); return; }
  const saved = await saveData();
  toast(saved ? `Добавлено в ${added} скрипт(ов)` : `Добавлено локально в ${added}`);
  render();
}
