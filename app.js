...
function searchCars(q) {
  ...
  return state.cars.filter(c => {
    const hay = [
      ...
      c.engine, c.power, c.fuel, c.bodyType || '', ...(c.tags || [])
    ].join(' ').toLowerCase();
    ...
    <div class="catalog-toolbar card">
      ...
      <div class="catalog-filters">
        <input type="search" class="search-input" id="catalogSearch"
          placeholder="Поиск: тайота, седан, кроссовер, vesta…"
...
async function ensureCallAudioLoaded(id, audioEl) {
  if (audioEl.dataset.loaded === '1' && audioEl.src) return;
...
async function toggleCallPlay(id) {
  ...
  // Если уже играет этот же трек — сразу пауза (без await)
  if (window._callAudioId === id && audio.src && !audio.paused) {
    audio.pause();
    btn.textContent = '▶';
    return;
  }
  ...
  if (audio.duration && durEl) durEl.textContent = formatDuration(audio.duration);
}
