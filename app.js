...
function nextPaydayWednesday(from = new Date()) {
  ...
  let add = (3 - day + 7) % 7;
  // если сегодня среда — следующая выплата сегодня (ещё не «прошла» для отображения)
  // для «осталось до выплаты» если уже после среды на этой неделе — +7
    ...
    <div class="card catalog-toolbar">
      <div class="catalog-toolbar-row">
        <div>
          ...
          <p class="catalog-hint">💸 Выплаты <b>каждую среду</b> · ближайшая: <b>${paydayIso.slice(5).replace('-', '.')}</b>${untilPay === 0 ? ' (сегодня)' : ' (через ' + untilPay + ' ' + pluralRu(untilPay, 'день', 'дня', 'дней') + ')'} · к выплате сейчас: <b>${formatMoney(plan.earnedTotal)}</b></p>
        </div>
