...
const SEARCH_FOCUS_IDS = ['homeSearch', 'searchInput', 'otabotkiSearch', 'catalogSearch', 'callsSearch', 'pickOtabotkaSearch'];
...
function countryMeta(code) {
  const c = (code || '').toUpperCase() || 'OTHER';
  return COUNTRY_LABELS[c] || { name: c, flag: '🌍' };
}
...
function renderCatalog() {
  ...
  if (countryFilter) {
    list = list.filter(c => (c.country || '').toUpperCase() === countryFilter.toUpperCase());
  }
  list = [...list].sort((a, b) => {
  ...
  const countryCounts = {};
  (state.cars || []).forEach(c => {
    const code = (c.country || 'OTHER').toUpperCase();
    countryCounts[code] = (countryCounts[code] || 0) + 1;
  });
  const countries = Object.keys(countryCounts).sort((a, b) => {
    const na = countryMeta(a).name.localeCompare(countryMeta(b).name, 'ru');
    return na;
  });
  ...
  <div class="catalog-layout">
    <aside class="catalog-country-panel" aria-label="Страны">
      ...
      ${countries.map(code => {
        ...
        const active = countryFilter.toUpperCase() === code;
...
function buildLeaderboard() {
  loadCallsMeta();
  loadLeaderboardData();
  const map = new Map(); // name -> { name, successes, penalties, funny, lastSuccessAt, lastAt, calls }

  function ensure(name) {
    if (!name) name = '—';
    if (!map.has(name)) {
      map.set(name, { name, successes: 0, penalties: 0, funny: 0, lastSuccessAt: 0, lastAt: 0, calls: 0 });
    }
    return map.get(name);
  }
  ...
  (state.calls || []).forEach(c => {
    ...
    row.calls += 1;
    row.lastAt = Math.max(row.lastAt, c.createdAt || 0);
    if (c.status === 'success' || c.status === 'success_penalty') {
      row.successes += 1;
      row.lastSuccessAt = Math.max(row.lastSuccessAt, c.createdAt || 0);
    }
    if (c.status === 'penalty' || c.status === 'success_penalty') row.penalties += 1;
    if (c.status === 'funny') row.funny += 1;
  });

  // manual adjustments from admin: { id, name, successesDelta, note, date }
  (state.leaderboardManual || []).forEach(m => {
    const row = ensure(m.name);
    row.successes += Number(m.successesDelta) || 0;
    if (m.date) {
      const t = new Date(m.date).getTime();
      if (isFinite(t)) row.lastSuccessAt = Math.max(row.lastSuccessAt, t);
    }
  });

  return [...map.values()]
    .filter(r => r.name !== '—')
    .sort((a, b) => b.successes - a.successes || b.lastSuccessAt - a.lastSuccessAt);
}
  ...
  return `
    <div class="card catalog-toolbar">
      <div class="catalog-toolbar-row">
        <div>
          ...
          <p class="catalog-hint">Успешные звонки по операторам. Данные из раздела «Звонки»${canAdmin ? ' + ручные правки администратора' : ''}.</p>
        </div>
        ${canAdmin ? `<button class="btn btn-primary btn-sm" data-action="add-lb-manual">+ Успех вручную</button>` : ''}
      </div>
    </div>
    ${rows.length === 0
            ...
            <tbody>
              ${rows.map((r, i) => `
                <tr class="${i === 0 ? 'lb-top' : ''}">
                  ...
                  <td><strong>${r.successes}</strong></td>
                  <td>${r.penalties}</td>
                  <td>${r.funny}</td>
                  <td>${r.calls}</td>
    ...
    ${canAdmin ? `
      <div class="card" style="margin-top:16px;padding:14px 16px">
        <h3 style="margin:0 0 10px;font-size:1rem">Ручные записи администратора</h3>
          ...
          : `<div class="team-list">${(state.leaderboardManual || []).slice().reverse().map(m => `
              <div class="team-row">
                <div>
                  ...
                  <span class="badge badge-teal">${m.successesDelta >= 0 ? '+' : ''}${m.successesDelta} успех</span>
...
function showLbManualModal() {
  ...
  openModal(
    ...
    `<div class="form-group"><label>Оператор</label>
     ...
     <div class="form-group"><label>Сколько успехов (+ или −)</label>
       <input type="number" id="fLbDelta" value="1" step="1">
     </div>
     <div class="form-group"><label>Дата</label>
       <input type="date" id="fLbDate" value="${new Date().toISOString().slice(0, 10)}">
     </div>
     <div class="form-group"><label>Комментарий</label>
       <input type="text" id="fLbNote" placeholder="Необязательно">
     </div>`,
    `<button class="btn btn-outline" data-action="close-modal">Отмена</button>
     <button class="btn btn-primary" data-action="save-lb-manual">Сохранить</button>`
  );
}

function saveLbManual() {
  if (!isAdminUser()) return;
  loadLeaderboardData();
  ...
  state.leaderboardManual.push({
    id: uid(), name, successesDelta: delta, date, note, createdAt: Date.now()
  });
...
function showViewCarModal(id) {
  ...
  openModal(
    ...
    `<div class="view-card">
      ...
      ${c.country ? `<div class="view-row"><span class="view-label">Страна</span><span class="view-value">${escapeHtml(c.country)}</span></div>` : ''}
...
function handleClick(e) {
  ...
  if (['add-team-user', 'edit-team-user', 'delete-team-user', 'save-team-user'].includes(action) && !isAdminUser()) {
  ...
  switch (action) {
    ...
    case 'set-catalog-country':
      ...
      break;
    case 'add-lb-manual': showLbManualModal(); break;
    case 'save-lb-manual': saveLbManual(); break;
