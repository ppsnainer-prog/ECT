...
async function cloudSave() {
  ...
  try {
    ...
    if (state.cloud.provider === 'sheets') {
      const url = (state.cloud.sheetsUrl || '').trim();
      if (!url) throw new Error('Нет URL Google Apps Script');
      // text/plain — чтобы не было CORS preflight к Apps Script
      ...
      // Apps Script иногда отвечает редиректом; если тело пустое — считаем успехом при 200
      let json = null;
      const textBody = await res.text();
      try { json = JSON.parse(textBody); } catch (_) {}
      if (!res.ok && !(json && json.ok)) {
        throw new Error('HTTP ' + res.status + (textBody ? (': ' + textBody.slice(0, 120)) : ''));
      }
