...
async function startApplication() {
  try {
    ...
    console.warn('loadData failed, continue with local data', e);
    try { loadLocalScripts(); } catch (_) {}
  }
  ...
  try {
    ...
    if (content) content.innerHTML = '<div class="empty-state"><p>Ошибка отрисовки. Обновите страницу.</p></div>';
  }
...
async function cloudFetch() {
  ...
  try {
    ...
    if (!res.ok) {
      if (res.status === 401) {
        toast('Ошибка 401 — нужен Master Key (не Access Key).', 'error');
      } else if (res.status === 403) {
        toast('Ошибка 403 — доступ к облаку запрещён.', 'error');
      }
