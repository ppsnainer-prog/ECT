        ...
        <article class="card ref-card">
          <div class="ref-card-head">
            ...
            <div class="call-actions">
              <button class="btn btn-outline btn-sm" data-action="view-ref" data-id="${escapeAttr(item.id)}">Открыть</button>
              ${canChange ? `
          ...
          ` : ''}
          ${item.body ? `<p class="ref-preview">${escapeHtml(String(item.body).slice(0, 180))}${item.body.length > 180 ? '…' : ''}</p>` : ''}
          <div class="call-meta">
  ...
  openModal(
    ...
    `<div class="form-group"><label>Заголовок</label>
     ...
     <div class="form-group"><label>Скриншоты / изображения</label>
       ...
       ${item && (item.images || []).length ? `<p class="field-hint">Уже загружено: ${item.images.length}. Новые файлы добавятся к ним.</p>` : ''}
     </div>
     <div class="form-group"><label>Аудио</label>
       <input type="file" id="fRefAudio" accept="audio/*,.mp3,.wav,.m4a,.ogg" multiple>
       ${item && (item.audio || []).length ? `<p class="field-hint">Уже загружено: ${item.audio.length}.</p>` : ''}
     </div>`,
    `<button class="btn btn-outline" data-action="close-modal">Отмена</button>
  ...
  try {
    if (imgInput?.files?.length) {
      for (const file of imgInput.files) {
        ...
        await idbPutAudio(mid, file);
        item.images.push({ id: mid, name: file.name, type: file.type });
      }
    }
    if (audInput?.files?.length) {
      for (const file of audInput.files) {
        const mid = REF_MEDIA_PREFIX + uid();
        await idbPutAudio(mid, file);
        item.audio.push({ id: mid, name: file.name, type: file.type });
      }
    }
  } catch (e) {
    console.warn(e);
    toast('Часть файлов не сохранилась (лимит браузера)', 'error');
  }
...
async function showRefView(id) {
  ...
  for (const img of (item.images || [])) {
    try {
      const blob = await idbGetAudio(img.id);
      if (blob) {
        const url = URL.createObjectURL(blob);
        imageHtml.push(`<figure class="ref-media"><img src="${url}" alt="${escapeAttr(img.name || '')}" class="ref-img"><figcaption>${escapeHtml(img.name || '')}</figcaption></figure>`);
      }
    } catch (_) {}
  }
  const audioHtml = [];
  for (const a of (item.audio || [])) {
    try {
      const blob = await idbGetAudio(a.id);
      if (blob) {
        const url = URL.createObjectURL(blob);
        audioHtml.push(`<div class="ref-audio"><div class="field-hint">${escapeHtml(a.name || 'Аудио')}</div><audio controls src="${url}" style="width:100%"></audio></div>`);
      }
    } catch (_) {}
  }

  openModal(
    escapeHtml(item.title || 'Материал'),
    `${item.tags && item.tags.length ? `<div class="ref-tags" style="margin-bottom:10px">${item.tags.map(t => `<span class="badge badge-primary">#${escapeHtml(t)}</span>`).join('')}</div>` : ''}
     ${item.body ? `<div class="ref-body">${escapeHtml(item.body).replace(/\\n/g, '<br>')}</div>` : ''}
     ${(item.links || []).length ? `
       <div style="margin-top:12px"><strong>Ссылки</strong>
         <ul class="rules-list" style="margin-top:8px">${item.links.map(l => `
           <li class="rules-item"><span class="rules-item-text">
    ...
    `<button class="btn btn-outline" data-action="close-modal">Закрыть</button>
     ${(!isCommonAccount() && canEdit()) ? `<button class="btn btn-primary" data-action="edit-ref" data-id="${escapeAttr(item.id)}">Редактировать</button>` : ''}`
  );
}
    ...
    <div class="catalog-main">
      <div class="card catalog-toolbar">
        ...
        <div class="catalog-filters">
          <div class="search-wrap" style="flex:1;min-width:180px">
            <span class="search-icon">🔍</span>
            <input type="search" id="rulesSearch" class="search-input" placeholder="Поиск в разделе…" value="${escapeAttr(state.rulesQuery || '')}" autocomplete="off">
          </div>
        </div>
      </div>
