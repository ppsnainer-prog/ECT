...
function buildLeaderboard(period) {
  ...
  return [...map.values()]
    .filter(r => r.name !== '—')
    .filter(r => r.calls > 0 || r.successes > 0 || r.penalties > 0)
    .sort((a, b) => {
          ...
          <table class="lb-table">
            ...
            <tbody>
              ${rows.map((r, i) => {
                ...
                const stats = `${calls} ${calls === 1 ? 'звонок' : (calls > 1 && calls < 5 ? 'звонка' : 'звонков')} — ${succ} ${succ === 1 ? 'успех' : (succ > 1 && succ < 5 ? 'успеха' : 'успехов')} — успешных дозвонов <strong>${pct}%</strong>`;
                return `
                <tr class="${i === 0 ? 'lb-top' : ''}">
