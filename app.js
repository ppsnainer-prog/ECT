...
const DEFAULT_RULES = [
  ...
  },
  {
    "id": "z0_fl",
    "category": "0%",
    "subtitle": "Заполнение / ответы клиента",
    "tone": "danger",
    "items": [
...
function renderRules() {
  ...
  let blocks = DEFAULT_RULES.map(b => ({ ...b, items: b.items.slice() }));
  ...
  if (q) {
    blocks = blocks.map(b => ({
      ...b,
      items: b.items.filter(it => it.toLowerCase().includes(q))
    })).filter(b => b.items.length);
  }

  const categories = [...new Set(DEFAULT_RULES.map(b => b.category))];
  const total = DEFAULT_RULES.reduce((s, b) => s + b.items.length, 0);
    ...
    <div class="card catalog-toolbar">
      <div class="catalog-toolbar-row">
        <div>
          ...
          <p class="catalog-hint">Критерии неактуальности и штрафов (0% / 50% / 75%). Разбито по блокам для быстрого поиска.</p>
        </div>
        <a class="btn btn-outline btn-sm" href="https://docs.google.com/spreadsheets/d/1rDcSs-C3e3kDMsqOwgCItyrpArxD1WK5rRHr5G9aEwI/edit?gid=1727881349#gid=1727881349" target="_blank" rel="noopener">Открыть таблицу</a>
      </div>
      <div class="catalog-filters">
        <div class="search-wrap" style="flex:1;min-width:180px">
          <span class="search-icon">🔍</span>
          <input type="search" id="rulesSearch" class="search-input" placeholder="Поиск по правилам…" value="${escapeAttr(state.rulesQuery || '')}">
        </div>
      </div>
      <div class="lb-period-tabs" style="margin-top:10px">
        <button type="button" class="lb-period-tab ${!cat ? 'active' : ''}" data-action="set-rules-category" data-cat="">Все · ${total}</button>
        ${categories.map(c => {
          const n = DEFAULT_RULES.filter(b => b.category === c).reduce((s, b) => s + b.items.length, 0);
      ...
      ${blocks.map(b => `
        <section class="card rules-block ${toneClass[b.tone] || ''}">
          ...
          <ul class="rules-list">
            ...
            `).join('') || '<li class="rules-item"><span class="rules-item-text">Нет совпадений</span></li>'}
          </ul>
        </section>
