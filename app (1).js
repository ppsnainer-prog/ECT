...
function loadLocalSettings() {
  ...
  try {
    ...
    if (rawC) {
      ...
      state.cloud.binId = c.binId || '';
      state.cloud.apiKey = c.apiKey || '';
      state.cloud.enabled = !!(c.binId && c.apiKey);
    }
  } catch (e) {}
}
...
function renderScriptDetail() {
  ...
  return `
    <div class="script-detail">
      ...
      <div class="tabs">
        <button class="tab ${tab === 'content' ? 'active' : ''}" data-action="tab" data-tab="content">📄 Текст</button>
        <button class="tab ${tab === 'otabotki' ? 'active' : ''}" data-action="tab" data-tab="otabotki">🔄 Отработки (${(script.otabotki || []).length})</button>
        <button class="tab ${tab === 'shtrafy' ? 'active' : ''}" data-action="tab" data-tab="shtrafy">⚠ Штрафы (${(script.shtrafy || []).length})</button>
      </div>
      ...
      <div class="tab-content ${tab === 'otabotki' ? 'active' : ''}">
        <div style="margin-bottom:16px">
          <button class="btn btn-primary btn-sm" data-action="add-item" data-id="${script.id}" data-type="otabotki">+ Добавить отработку</button>
        </div>
        ${(script.otabotki || []).length === 0
          ? '<div class="empty-state"><p>Отработок пока нет</p></div>'
          : `<div class="item-list">${(script.otabotki || []).map(item => `
      ...
      <div class="tab-content ${tab === 'shtrafy' ? 'active' : ''}">
        <div style="margin-bottom:16px">
          <button class="btn btn-primary btn-sm" data-action="add-item" data-id="${script.id}" data-type="shtrafy">+ Добавить штраф</button>
        </div>
        ${(script.shtrafy || []).length === 0
          ? '<div class="empty-state"><p>Штрафов пока нет</p></div>'
          : `<div class="item-list">${(script.shtrafy || []).map(item => `
